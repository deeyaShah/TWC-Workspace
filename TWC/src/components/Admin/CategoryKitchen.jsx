...
const CategoryKitchen = () => {
  ...
  const fetchMainCategoryId = async () => {
    try {
      ...
      if (kitchenCategory) {
        setMainCategoryId(kitchenCategory._id);
        return kitchenCategory._id;
      } else {
        message.error("Modular Kitchen category not found.");
        return null;
      }
    } catch (err) {
      console.error(err);
  ...
  //   const toEdit = kitchenCategories.find((cat) => cat._id === id);
  ...
  const handleRowClick = (record) => {
    navigate(`/admin/dashboard/category/kitchen/${record._id}`);
  };
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-4">Manage Modular Kitchen Categories</h2>
      ...
      {loading ? (
        ...
        <Table
          columns={columns}
          dataSource={kitchenCategories.map((cat) => ({ ...cat, key: cat._id }))}
          pagination={{
            current: currentPage,
            pageSize,
